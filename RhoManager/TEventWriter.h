...
class  TEventWriter: public VAbsWriter {
public:
    
    TEventWriter();
    TEventWriter(const char *file); 
    TEventWriter(VAbsCollection *collection); 
    virtual ~TEventWriter();
    
    virtual void WriteEvent(TEventManager *evtmgr, TCandList *list=0);
...
private :
    ...
    Bool_t fWriter, fWriteCollection; //!
...
public:
    ClassDef(TEventWriter,1) // Base class to write ROOT data files
};
