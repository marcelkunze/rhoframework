...
void NetworkTrainer::SetupDataServer(const char* file)
{
	...
	for (i=0;i<fAll.GetSize();i++) {
	    ...
	    fPidDataServer->TTreeDataRead(fileName.c_str(),fTree,fInput,fOutput,fCut);
	}
	for (i=0;i<fPro.GetSize();i++) {
	    string fileName = path + (((TObjString*)fPro.At(i))->GetString()).Data();
	    cout << "+sample " << fileName << endl;
	    fPidDataServer->TTreeDataRead(fileName.c_str(),fTree,fInput,"1",fCut);
	}
	for (i=0;i<fCon.GetSize();i++) {
	    string fileName = path + (((TObjString*)fCon.At(i))->GetString()).Data();
	    cout << "-sample " << fileName << endl;
	    fPidDataServer->TTreeDataRead(fileName.c_str(),fTree,fInput,"0",fCut);
	}
    }
    else {
	...
	string stdinput(fInput);
	TString input(stdinput.c_str());
	input.ReplaceAll(":"," ");
	input += " ";
...
Bool_t NetworkTrainer::ReadSteeringFile(const char *filename)
{
	...
	if (key == "") { s.getline(skip,len); } // empty line
	    ...
	    string networkpath;
	    s >> networkpath;
	    fNetworkPath = networkpath.c_str();
	    ...
	    string pro;
	    s >> pro;
	    fPro.Add(new TObjString(pro.c_str()));
	    cout << "+pro " << pro << endl;
	}
	    ...
	    string con;
	    s >> con;
	    fCon.Add(new TObjString(con.c_str()));
	    cout << "+con " << con << endl;
	}
	    ...
	    string all;
	    s >> all;
	    fAll.Add(new TObjString(all.c_str()));
	    cout << "+file " << all << endl;
	}
