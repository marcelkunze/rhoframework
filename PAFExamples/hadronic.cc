// Filter macro for hadronic events in the PAF
...
Int_t hadronic(int argc=0,char* argv[]=0)
{
    ...
    TRho PAF("Hadronic filter");                   // Instantiate a named framework
    cout<<PAF;
    ...
    TPersistenceManager* persmgr = PAF.RegisterService(new TPersistenceManager());
    ...
    TCandListManager* listmgr = PAF.RegisterService(new TCandListManager());
